...
int main(int argc, char* argv[])
{
	...
	if (correlation_buff!= NULL){
			printf("correlation_buff is allocated.\n");
	} else {
		return -1;
	}

	if (correlation_sw != NULL) {
...
//    TIME_STAMP_SW;
    ...
    sds_free(indices_buff);
    sds_free(correlation_buff);
    sds_free(correlation_sw);
    return 0;
}
