...
static void mmultadd_init(float *tin1Buf, float *tin2Buf, float *tin3Buf,
                          float *toutBufSw, float *toutBufHw)
{
  int i, j;
  ...
  for (i = 0; i < B_NROWS; i++) {
    for (j = 0; j < B_NCOLS; j++) {
      tin3Buf[i * B_NCOLS + j] =  i*A_NCOLS-j;
    }
  }
  for (i = 0; i < A_NROWS; i++) {
    for (j = 0; j < B_NCOLS; j++) {
...
int mmultadd_test(float *tin1Buf,  float *tin2Buf, float *tin3Buf,
                  float *toutBufSw, float *toutBufHw)
{
  int i;

  float *tmpBuf1 = (float *)sds_alloc(A_NROWS * A_NCOLS * sizeof(float));
  float tmpBuf2[A_NROWS * A_NCOLS];
  
  printf("Testing mmult ...\n");
  
  mmultadd_init(tin1Buf, tin2Buf, tin3Buf, toutBufSw, toutBufHw);
  
  TIME_STAMP_INIT
    
  for (i = 0; i < NUM_TESTS; i++) {
    mmult_golden(tin1Buf, tin2Buf, tmpBuf1);
    madd_golden(tmpBuf1, tin3Buf, toutBufSw);
  }

  TIME_STAMP_SW

  for (i = 0; i < NUM_TESTS; i++) {
    _p0_mmult_0(tin1Buf, tin2Buf, tmpBuf2);
    //printf("tmpBuf2[0] = %f\n", tmpBuf2[0]);
    _p0_madd_0(tmpBuf2, tin3Buf, toutBufHw);
  }
