...
FIL fil_in, fil_out;		/* File object */
char *SD_in, *SD_out;
FRESULT Res;
...
int write_frames(int frames, int rows, int cols, int chroma)
{
  ...
  for (frame = 0; frame<frames; frame++) {
    ...
    if (write_yuv8(&fil_out, &(yuv8_video_out))>0) {
      exit(1);
    }
  }
...
int read_frames(int frames, int rows, int cols, int chroma)
{
  ...
  for (frame = 0; frame<frames; frame++) {
    ...
    if (read_yuv8(&fil_in, &(yuv8_video_in))>0) {
      exit(1);
    }
    printf("Copying to video\n");
...
#define CHROMA 2
  ...
  frames      =  10; // atoi( argv[2] );
  nr_strength =  2; // atoi( argv[3] );
  if (frames > MAX_FRAMES)
    frames = MAX_FRAMES;
  
  read_frames(frames, rows, cols, CHROMA);
  Xil_DCacheEnable();
  process_frames(frames, nr_strength);
  Xil_DCacheFlush();
  Xil_DCacheDisable();
  write_frames(frames, rows, cols, CHROMA);
  free_frames(frames);
  f_close(&fil_in);
  f_close(&fil_out);
  return(0);
