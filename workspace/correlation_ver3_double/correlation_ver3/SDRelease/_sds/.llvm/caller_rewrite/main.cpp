...
int main(int argc, char* argv[])
{
	...
	*----------------------------------------------------------*/
    TIME_STAMP_INIT;
    int ptr_result_1 = 0;
    int ptr_result_2 = number_of_indices - 1;
    for(int i = number_of_indices; i > 1; i = i - 2){
		#pragma SDS async(1)
        _p0_correlation_accel_v2_1(
                                number_of_days_per_index,   /* CPU in*/
                                i,          /* CPU in*/
                                (float *)(indices_buff + (number_of_indices - i) * number_of_days_per_index),  /*  Input*/
                                (float *)(correlation_buff + ptr_result_1)        	/* Output*/
        );
        ...
        _p0_correlation_accel_v2_0(
								number_of_days_per_index,   /* CPU in*/
								i - 1,          /* CPU in*/
								(float *)(indices_buff + (number_of_indices - i + 1) * number_of_days_per_index),  /*  Input*/
								(float *)(correlation_buff + ptr_result_2)        	/* Output*/
		);
  sds_wait(1); //rewritten from #pragma SDS wait(1)
        sds_wait(2);
        ptr_result_1 += (i - 1) + (i - 2);
        ptr_result_2 += (i - 2) + (i - 3);
    }
    TIME_STAMP_ACCEL;
	/************************************************************
